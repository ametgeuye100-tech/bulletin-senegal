...
{% for b in bulletins %}<div style="border:1px solid #ccc;padding:10px;margin:10px 0;"><b>{{b.periode}} - {{b.annee}}</b> | Moyenne: <b>{{b.moyenne}}/20</b><br>{{b.appreciation}}</div>
...
<hr>
<table><tr><th>N°</th><th>DISCIPLINES</th><th>D1</th><th>D2</th><th>Comp</th><th>Coef</th><th>Appréciation</th></tr>
{% for i in range(20) %}
<tr>
<td>{{ i+1 }}</td>
<td><select name="matiere{{i}}" class="matiere-select" style="width:200px"></select></td>
<td><input name="devoir1_{{i}}" type="number" step="0.01" onkeyup="calculer({{i}})"></td>
<td><input name="devoir2_{{i}}" type="number" step="0.01" onkeyup="calculer({{i}})"></td>
<td><input name="comp{{i}}" type="number" step="0.01" onkeyup="calculer({{i}})"></td>
