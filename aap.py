...
def voir_bulletin(id):
    ...
    points_dispo = round(somme_coeff * 20, 2) # POINTS DISPONIBLES = SOMME COEF * 20
    mention = get_mention_generale(bulletin.moyenne)

    return render_template_string(BULLETIN_OFFICIEL_HTML, bulletin=bulletin, eleve=eleve, matieres=matieres, date_jour=date.today().strftime("%d/%m/%Y"), somme_points=somme_points, somme_coeff=somme_coeff, points_dispo=points_dispo, mention=mention)
