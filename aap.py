...
MATIERES = {
    ...
    "3e": ["FRANÇAIS", "MATHS", "PC", "SVT", "HISTOIRE/GEOGRAPHIE", "ANGLAIS", "EPS", "EC", "ESPAGNOL", "DESSIN"],
    "Terminale S": ["PHILOSOPHIE", "FRANÇAIS", "HISTOIRE/GEOGRAPHIE", "ANGLAIS", "MATHS", "PC", "SVT", "EPS"]
}
...
LOGIN_HTML = """<!DOCTYPE html><html><head><title>Connexion</title><style>body{font-family:Arial;background:#00853F;display:flex;justify-content:center;align-items:center;height:100vh}input{text-transform:uppercase}.box{background:white;padding:30px;border-radius:10px;width:350px}h2{text-align:center;color:#00853F}input,select{width:100%;padding:10px;margin:8px 0}button{background:#00853F;color:white;padding:12px;border:none;width:100%;border-radius:5px;font-size:16px;cursor:pointer}</style></head><body><div class="box"><h2>🇸🇳 BULLETIN SENEGAL 🇸🇳</h2><form method="POST" action="/inscription"><label>Téléphone:</label><input name="telephone" required><label>Prénom:</label><input name="prenom" required><label>Nom:</label><input name="nom" required><label>Classe:</label><select name="classe">{% for c in classes %}<option>{{c}}</option>{% endfor %}</select><button>ENTRER</button></form></div></body></html>"""
...
function confirmerSuppression(id){ if(confirm("Tu es sûr de vouloir supprimer ce bulletin?")){ window.location.href='/supprimer/'+id; } }
...
function calculer(i){let d1=parseFloat(document.getElementsByName('devoir1_'+i)[0].value)||0;let d2=parseFloat(document.getElementsByName('devoir2_'+i)[0].value)||0;let comp=parseFloat(document.getElementsByName('comp'+i)[0].value)||0;let moy=((d1+d2+comp*2)/4).toFixed(2);document.getElementsByName('app_mat'+i)[0].value=get_appreciation_matiere(moy);}
function get_appreciation_matiere(moy){if(moy>=18)return'Excellent';else if(moy>=16)return'Très Bien';else if(moy>=14)return'Bien';else if(moy>=12)return'Assez Bien';else if(moy>=10)return'Passable';else return'Insuffisant';}
...
<b>Moyenne:</b> <span style="font-size:18px;color:#E31B23">{{b.moyenne}}/20</span> <br>
<b>Appréciation:</b> {{b.appreciation}} <br><br>
<a href="/bulletin/{{b.id}}" class="btn-blue">📄 VOIR BULLETIN</a>
<button onclick="confirmerSuppression({{b.id}})" class="btn-red">🗑️ SUPPRIMER</button>
</div>
{% else %}<p>Aucun bulletin.</p>{% endfor %}<hr>
...
<label>Etablissement:</label><input name="etablissement" value="LYCEE">
...
<hr>
<table><tr><th>N°</th><th>DISCIPLINES</th><th>D1</th><th>D2</th><th>Comp</th><th>Coef</th><th>App</th></tr>
{% for i in range(20) %}
<tr>
<td>{{ i+1 }}</td>
...
<td><input name="app_mat{{i}}" readonly style="background:#ddd;width:100px"></td>
</tr>
{% endfor %}
</table><br><button class="btn-green">ENREGISTRER</button></form>
<script>updateMatieres();</script>
</div></body></html>"""

BULLETIN_OFFICIEL_HTML = """<!DOCTYPE html><html><head><title>Bulletin</title><style>
body{font-family:'Times New Roman';padding:20px;background:#fff;font-size:12px}.cadre{border:4px double #00853F;padding:15px}
.header{text-align:center;margin-bottom:10px}.drapeau-img{height:80px;margin-bottom:10px} /* 1 SEUL DRAPEAU */
.titre{color:#00853F;font-weight:bold}.info{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:15px 0;border-bottom:2px solid #000;padding-bottom:10px}
...
<div class="header">
<img src="https://flagcdn.com/w320/sn.png" class="drapeau-img"> <!-- UN SEUL DRAPEAU ICI -->
...
<div><b>Etablissement:</b> {{bulletin.etablissement}}</div><div><b>Année:</b> {{bulletin.annee}}</div>
...
{% endfor %}</table>
<div class="total-box">
<div>SOMME COEF<br>{{somme_coeff}}</div>
<div>SOMME NOTES<br>{{somme_notes}}</div>
<div>NOTES DISPO<br>{{notes_dispo}}</div>
<div>POINTS<br>{{somme_points}}</div>
<div>MOYENNE<br>{{bulletin.moyenne}}/20</div>
</div>
<div style="display:flex;justify-content:space-between;margin-top:20px">
<div class="mention-box">MENTION: {{mention}}</div>
<div style="border:1px solid #000;padding:10px;width:50%"><b>APPRECIATION:</b><br> {{bulletin.appreciation}}</div>
</div>
</div><button class="no-print" onclick="window.print()" style="padding:10px 20px;background:#00853F;color:white;border:none;border-radius:5px;margin-top:10px">🖨️ IMPRIMER</button></body></html>"""
...
def voir_bulletin(id):
    ...
    return render_template_string(BULLETIN_OFFICIEL_HTML, bulletin=bulletin, eleve=eleve, matieres=matieres, somme_points=somme_points, somme_coeff=somme_coeff, somme_notes=somme_notes, notes_dispo=notes_dispo, mention=mention)
