...
.boite{margin-top:20px;padding:15px;border:2px solid #00853F;border-radius:8px}
</style><script>
...
let moy=((d1+d2+comp*2)/4).toFixed(2); // 2 DEVOIRS + 2 COMP
...
<hr>
<table><tr><th>N°</th><th style="width:140px">DISCIPLINES</th><th>DEVOIR 1</th><th>DEVOIR 2</th><th>COMPOS</th><th>COEF</th><th>MOY</th></tr>
{% for i in range(20) %}
<tr>
<td>{{ i+1 }}</td>
...
<a href="/dashboard" class="btn">RETOUR</a><h2>📦 BOITE BULLETINS - {{eleve.prenom}} {{eleve.nom}}</h2>
<p>Tous vos bulletins prêts à imprimer</p>
...
<td><a href="/bulletin/{{b.id}}" class="btn-blue">IMPRIMER PDF</a></td>
</tr>
{% endfor %}
</table>
...
<table>
<tr>
<th>Disciplines</th><th>Devoir 1</th><th>Devoir 2</th><th>Compos</th><th>Moy</th><th>Coeff</th><th>MoyxCoeff</th><th>Rang</th><th>Appréciations</th>
</tr>
{% for i, m in enumerate(matieres) %}
<tr>
<td class="left">{{m.nom}}</td>
<td>{{m.d1}}</td><td>{{m.d2}}</td><td>{{m.comp}}</td><td>{{m.moy}}</td><td>{{m.coef}}</td><td>{{m.total}}</td><td>{{i+1}}ex</td><td>{{m.app}}</td>
</tr>
{% endfor %}
<tr class="totaux">
<td class="left">Totaux</td><td></td><td></td><td></td><td></td><td>{{somme_coeff}}</td><td>{{somme_points}}</td><td></td><td></td>
</tr>
</table>
...
def saisie():
    ...
    for i in range(20):
        ...
        if matiere and matiere!= "--CHOISIR--":
            ...
            if d1>0 or d2>0 or comp>0:
                ...
                matieres_calculees.append({"nom":matiere.upper(),"d1":d1,"d2":d2,"comp":comp,"moy":moy,"coef":coeff,"total":total,"app":app_mat})
    ...
    return redirect(url_for('mes_bulletins')) # Redirige vers la boite
