...
body{font-family:'Times New Roman';padding:20px;background:#fff}.cadre{border:3px solid #00853F;padding:10px}
.header{text-align:center}.drapeau{font-size:40px}.info{display:grid;grid-template-columns:1fr 1fr;gap:5px;margin:10px 0}
table{width:100%;border-collapse:collapse;margin-top:10px}td,th{border:1px solid #000;padding:5px;text-align:center}
.footer{margin-top:20px;display:flex;justify-content:space-between}
@media print{.no-print{display:none}} </style></head><body><div class="cadre">
<div class="header"><div class="drapeau">🇸🇳</div><h2>REPUBLIQUE DU SENEGAL</h2>
<h3>MINISTERE DE L'EDUCATION NATIONALE</h3><h4>BULLETIN DE NOTES</h4></div><hr>
<div class="info">
...
<div><b>Période:</b> {{bulletin.periode}}</div><div><b>Moyenne:</b> {{bulletin.moyenne}}/20</div>
</div>
<table><tr><th>DISCIPLINES</th><th>MOY/20</th><th>COEF</th><th>TOTAL</th><th>APPRECIATION</th></tr>
{% for m in matieres %}
<tr><td>{{m.nom}}</td><td>{{m.moy}}</td><td>{{m.coef}}</td><td>{{m.total}}</td><td>{{m.app}}</td></tr>
{% endfor %}</table>
<p><b>Appréciation Générale:</b> {{bulletin.appreciation}}</p>
<div class="footer"><div>Fait à Dakar, le {{annee_actuelle}}</div><div>Le Proviseur</div></div>
</div><button class="no-print" onclick="window.print()">🖨️ IMPRIMER / ENREGISTRER EN PDF</button></body></html>"""
...
def voir_bulletin(id):
    ...
    for m in matieres: m['app'] = get_appreciation_matiere(m['moy'])
    from datetime import date
    return render_template_string(BULLETIN_OFFICIEL_HTML, bulletin=bulletin, eleve=eleve, matieres=matieres, annee_actuelle=date.today().year)
