...
table{width:100%;border-collapse:collapse;margin-top:10px}th{background:#00853F;color:white}td,th{border:1px solid #000;padding:6px;text-align:center}
.total-box{display:flex;justify-content:space-between;margin-top:15px;padding:10px;border:2px solid #E31B23;background:#FFF0F0}
...
<div class="total-box">
<div>
<b>Appréciation Générale:</b><br> {{bulletin.appreciation}}
</div>
<div style="text-align:right">
<b>SOMME DES POINTS:</b> {{somme_points}} <br>
...
def voir_bulletin(id):
    ...
    for m in matieres:
        ...
        somme_points += m['total']
    somme_points = round(somme_points, 2)
    return render_template_string(BULLETIN_OFFICIEL_HTML, bulletin=bulletin, eleve=eleve, matieres=matieres, date_jour=date.today().strftime("%d/%m/%Y"), somme_points=somme_points)
